...
public:
                 ...
                 int max_iters = 30) const          //LM最大迭代次数
    {
        ...
        else tz_est = (r_img > 1.0) ? (f_mean * real_r / r_img) : 200.0;    //小孔成像原理计算深度
...
private:
    ...
    //LM 做的就是不断改 yaw,pitch,tx,ty,(tz)，让这些残差都趋近 0。
                         ...
                         //常成员函数：不修改类里面的任何成员变量
    {
        ...
        if (tz < 100.0 || targetEllipse.size.width < 2.0 || targetEllipse.size.height < 2.0) {   //深度非常小 或者  椭圆尺寸异常小   返回一个很大的残差，表示计算错误
        ...
        for (const auto& p2 : proj_outer) {   //对每个投影点计算 椭圆方程误差
